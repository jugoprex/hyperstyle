...
p = Path('./Directions/all/')
npz = p.glob('*.npz')
p2 = Path('./models/LR/')
sav = p2.glob('*.sav')
npz_vect = [torch.tensor(np.load(x)['w']).reshape([18,512]).cuda() for x in sorted(npz)] 
sav_vect = [x for x in sorted(sav)]
vect = npz_vect[0]

inicial = torch.tensor(vect, device=torch.device('cuda'))  
for j in range(len(sav_vect)):
    loaded_model = pickle.load(open(sav_vect[j], 'rb'))
    result = loaded_model.predict(vect.reshape([9216,1]).cuda().detach().cpu().numpy())
    ...
    for i in range(10):  
            ...
            np.savez(f'./results/latente/{i}_{j}.npz', w=lat[0].cpu())   
            ...
            res = (lat_image[0])[0]     
            outputs_path = fr'./results/imagen/{j}_{i}.jpg'
